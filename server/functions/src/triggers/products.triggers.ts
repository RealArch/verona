...
import { updateCategoryProductCounters, moveCategoryProductCounter } from '../utils/categoryCounters';
...
export const onProductCreated = onDocumentCreated(
    ...
    async (event) => {
        ...
        // Actualizar contadores globales de productos
        ...
        // Actualizar contadores de categoría (ejecutar en paralelo, sin await)
        if (data.categoryId) {
            updateCategoryProductCounters(data.categoryId, 1).catch(error => {
                console.error(`[ProductTrigger] Error updating category counters for product ${productId}:`, error);
            });
        }
    }
);
...
export const onProductUpdated = onDocumentUpdated(
    ...
    async (event) => {
        ...
        // Manejar cambios de categoría (ejecutar en paralelo, sin await)
        const categoryChanged = before.categoryId !== after.categoryId;
        if (categoryChanged && before.categoryId && after.categoryId) {
            moveCategoryProductCounter(before.categoryId, after.categoryId).catch(error => {
                console.error(`[ProductTrigger] Error moving product ${productId} between categories:`, error);
            });
        }
    }
);
...
export const onProductDeleted = onDocumentDeleted(
    ...
    async (event) => {
        ...
        if (data.categoryId) {
            updateCategoryProductCounters(data.categoryId, -1).catch(error => {
                console.error(`[ProductTrigger] Error updating category counters for product deletion ${event.params.productId}:`, error);
            });
        }
    });
